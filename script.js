document.addEventListener('DOMContentLoaded', () => {         //HINT : HTML Document 

    // --- CURSOR & RIPPLES ---
    const cursor = document.getElementById('game-cursor');           //HINT : Target element by its id
    document.addEventListener('mousemove', (e) => {
        cursor.style.left = `${e.clientX - 10}px`; cursor.style.top = `${e.clientY - 10}px`;         // HINT : Add Units for the offsets
    });
    document.querySelectorAll('.interactable').forEach(el => {
        el.addEventListener('mouseenter', () => cursor.classList.add('hovering'));
        el.addEventListener('mouseleave', () => cursor.classList.remove('hovering'));
    });
    document.addEventListener('click', (e) => {              //HINT : Mouse clicked event
    ...
    window.addEventListener('resize', resizeCanvas);     //HINT : call resizeCanvas function
    
    resizeCanvas();     //HINT : call resizeCanvas function
    
    for (let i = 0; i < 30; i++) {                 //HINT : Range should go from 0 till 30
    ...
    function animateParticles() {
        ...
        requestAnimationFrame(animateParticles);        //HINT : Call requestAnimationFrame function
    }
    animateParticles();                        //HINT : Call animateParticles function
    ...
    function fireConfetti() {     //HINT : define fireConfetti function
        ...
        animateConfetti();                    //HINT : Call animateConfetti function
    }
    function animateConfetti() {             //HINT : define animateConfetti function
    ...
    let gameActive = false;               //HINT : Set value to NO
    ...
    // 3D Camera
    document.addEventListener('mousemove', (e) => {       //Hint : Event where mouse moves
        if (!gameActive) return;            //HINT : if GAME IS NOT ACTIVE (refer to gameActive variable), return nothing
    ...
    puzzleInput.addEventListener('keypress', (e) => { if (e.key === 'ENTER') checkPuzzle(); });        //HINT : ENTER key clicked

    function checkPuzzle() {                //HINT : Define checkPuzzle function
        ...
        if (guess === gameData[activeDoorId].answer) {
            ...
            if (activeDoorId < 3) {        //HINT : if activeDoorId is less than 3
                ...
                nextDoor.querySelector('.status-icon').innerText = "[ unlocked ]";           //HINT : Text that appears on the door when it is UNLOCKED
                missionText.innerText = `MISSION: UNLOCK DOOR ${activeDoorId + 1}`;
            }

            // Swap modals
            puzzleModal.style.display = 'none';
            openContentModal(targetContent);     //HINT : call openContentModal function
    ...
    function openContentModal(targetId) {        //HINT : define openContentModal function
            ...
            setTimeout(() => {
                ...
                if (finalDoor.classList.contains('solved') && btn.classList.contains('close-btn')) {
                    ...
                    if (missionText.innerText !== "MISSION: COMPLETE") {
                        ...
                        fireConfetti();       //HINT : call fireConfetti function
